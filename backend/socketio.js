...
if (!MEDIASOUP_ANNOUNCED_IP) {
    console.warn(
        "[mediasoup] MEDIASOUP_ANNOUNCED_IP is not set. Remote clients may connect to signaling but fail to receive media.",
    );
}
...
const addProducer = (producer, roomId, socketId, kind) => {
    producers = [...producers, { socketId, roomId, producer, kind }];
    peers[socketId] = {
        ...peers[socketId],
...
const addConsumer = (consumer, roomId, socketId) => {
    consumers = [...consumers, { socketId, roomId, consumer }];
    peers[socketId] = {
        ...peers[socketId],
    ...
    peersToInform.forEach((peerSocketId) => {
        ...
        if (peerSocketId != socketId && peers[socketId].producers.length == 2) {
            peers[peerSocketId].socket.emit("new-producer", {
                newProducers: peers[socketId].producers,
                i: ++i,
            });
        }
    });
...
const getProducerTransport = (socketId) => {
    const [producerTransport] = transports.filter(
        (transportData) => transportData.socketId == socketId && !transportData.consumer,
    );
...
const socketConnection = (socket) => {
    ...
    socket.on(
        ...
        async ({ rtpCapabilities, serverConsumerTransportId, remoteProducerId }, cb) => {
            const { roomId } = peers[socket.id];
    ...
    socket.on("disconnect", () => {
        if (peers[socket.id]) {
            ...
            if (roomId && rooms[roomId]) {
                rooms[roomId].peers = rooms[roomId].peers.filter(
